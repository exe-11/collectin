...
import uz.itransition.collectin.exception.FileManagementException;
...
public class FileService {
    ...
    public String uploadFile(MultipartFile file) {
        try{
            ...
            throw FileManagementException.upload();
        }
    }

    public boolean delete(String path) {
        try {
            final String fileUniqueId = this.getFileNameFromPath(path);
            this.deleteFile(fileUniqueId);
            return true;
        } catch (Exception e) {
            throw FileManagementException.deletion();
        }
    }

    private boolean deleteFile(String fileUniqueId){
